...
# Function 2: Takes the pre-processed and interpreted data and analyzes the queried bug-drug combinations
...
amr_grp1_analysis= function(org_name, abs_ref, cntry, par, par_var_name){
  ...
  #subgroups
    ...
    group_by(across(all_of(c('mo_organism', par_var_name)))) %>%
  ...
  plt_hold <- ggplot(hold_df_plot %>% drop_na(ab), aes(x=.data[[paste0(par_var_name)]], y=values*100, group=ab, fill=ab))+
  ...
  write.csv(hold_df, paste0('Results/',cntry,'_',org_name,'_overall.csv'))

  hold_df_plot <- as.tibble(t(hold_df))%>%
    mutate(variable=names(hold_df),
           values=V1,
           ab=ab_name(ifelse(grepl('total', variable), str_split_i(variable, '_', 1), '')))

  plt_hold <- ggplot(hold_df_plot %>% drop_na(ab), aes(x=ab, y=values*100, group=ab))+
  ...
  ggsave(paste0('plots/',cntry,'_',org_name,'_overall','.png'), plt_hold, width=8, height=8, units="in", dpi=300)
